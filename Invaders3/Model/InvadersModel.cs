...
namespace Invaders3.Model
{
    class InvadersModel
    {
        ...
        private readonly List<Invader> _invaders = new List<Invader>();  // _player = new Player();
        ...
        {
            Point point = new Point(_random.Next((int)PlayAreaSize.Width), _random.Next(20, 
                                    (int)PlayAreaSize.Height) - 20);
                ...
                for (int column = 0; column < 11; column++)
                {
                    ...
                    switch (row)
                    {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            invader = new Invader(InvaderType.Flyingsaucer, location, 30);
                            break;
                        case 3:
        ...
        private void CheckForPlayerCollisions()
        {
            ...
            var result = 
                ...
                where invader.Area.Bottom > _player.Area.Top  // the buttom of the battlefield
                select invader;
            if (result.Count() > 0)  // reached the button of the battlefield
                EndGame();

            var shotsHit =
              from shot in _playerShots  // ???
              where _player.Area.Contains(shot.Location) == true && shot.Direction == Direction.Down
              select shot;
        ...
        private void CheckForInvaderCollisions()
        {
            ...
            foreach (Shot shot in _playerShots)
            {
                var result = from invader in _invaders
                             where invader.Area.Contains(shot.Location) == true && 
                                   shot.Direction == Direction.Up
            ...
            {
                Score += invader.Score;
                _invaders.Remove(invader);  // 1
                OnShipChanged(invader, true);   // 2
            }
        ...
        private void MoveInvaders()
        {
            ...
            if (DateTime.Now - _lastUpdated > TimeSpan.FromMilliseconds(millisecondsBetweenMovements))
            {
                ...
                var invadersTouchingLeftBoundary = 
                    from invader in _invaders
                    where invader.Area.Left < Invader.HorizontalInterval  // down here ?
                    select invader;
        ...
        internal void UpdateAllShipsAndStars()  // Page 2 back
        {
            foreach (Shot shot in _playerShots)
                OnShotMoved(shot, false);
